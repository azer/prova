...
function createProxy (routes, param) {
  ...
  routes[pattern] = onProxyRequest
  routes[pattern + '/'] = onProxyRequest
  routes[pattern + '/:filename([\\w\\.\\/-]+)'] = onProxyRequest

  function onProxyRequest (req, res, match) {
    ...
    var proxy = request[method](target + url)
    req.pipe(proxy)
    proxy.pipe(res).on('error', function (error) {
