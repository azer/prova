...
var browserTemplate = nodeTemplate('browser');
  ...
  server.on('upgrade', function(request, socket, body) {
    ...
    if (WebSocket.isWebSocket(request)) {
      ...
      ws.on('message', function (msg) {
        ...
        if (msg.result) return outputResult(msg);
      });
...
function outputResult (msg) {
  ...
  var rawUserAgent = msg.userAgent;
  var userAgent = parseUserAgent(rawUserAgent);
  var template = failed ? failTemplate : passTemplate;

  console.log(formatText(template, msg.result) + formatText(browserTemplate, {
